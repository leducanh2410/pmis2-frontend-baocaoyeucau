...
export class DashboardDetailComponent implements OnInit, OnDestroy {
    ...
    if (this._router.url === URL.CREATE) {
      ...
      // this._dashboardService.changeVisibility
        ...
        this._route.params.subscribe((response: any) => {
          ...
          this._dashboardService.getDashboardByDashboardId(this.dashboardId).subscribe( db => {
            if (db.status == 1) {
              this.loadData(db.data);
            } else {
              this._messageService.showErrorMessage(
                'Thông báo',
                db.message
  ...
  loadData(data: any): void {
    ...
    for (
      let index = 0;
      index < lstDashboardChartsPosition.length;
      index++
    ) {
      const element = lstDashboardChartsPosition[index];
      let tmp = this.charts.filter(
        (e) => e.chartId === lstDashboardCharts[index]
      )[0];
      if (element === Frame.F1) {
  ...
  onSearching() {
    this.charts = this.dataSource.filter((e) =>
      e.name.toUpperCase().includes(this.searchingValue.toUpperCase())
    );
  }
    ...
    dialogRef.afterClosed().subscribe((response: string) => {
      if (
        ...
        if (response == LayoutType.LT1) {
          ...
          response == LayoutType.LT2 ||
          response == LayoutType.LT3 ||
          response == LayoutType.LT4
        ) {
  ...
  selectedLayoutFunc(): boolean {
    if (
      this.selectedLayout == LayoutType.LT1 ||
  ...
  cancelChart(item: Chart, frame: string): void {
    if (frame == Frame.F1) {
      this.lstChartsF1 = this.lstChartsF1.filter(
        (e) => e.chartId != item.chartId
      );
    } else if (frame == Frame.F2) {
      this.lstChartsF2 = this.lstChartsF2.filter(
        (e) => e.chartId != item.chartId
      );
    } else {
      this.lstChartsF3 = this.lstChartsF3.filter(
        (e) => e.chartId != item.chartId
      );
    }
  ...
  onSaveDashboard(): void {
    ...
    if (
      this.lstChartsF1.length <= 0 &&
      this.lstChartsF2.length <= 0 &&
      this.lstChartsF3.length <= 0
    ) {
      this._messageService.showErrorMessage(
        'Thông báo',
        'Dashboard không chứa biểu đồ nào'
      );
      return;
    }
     ...
     this._dashboardService
        ...
        .subscribe((response: any) => {
          if (response.status == 1) {
            ...
            this._messageService.showErrorMessage(
              'Thông báo',
              'Đã xảy ra lỗi khi tạo dashboard'
            );
          }
        });
    } else if (this.mode == MODE.EDIT) {
  ...
  onDeleteDashboard() {
    ...
    this._messageService.showConfirm(
      ...
      (toast: SnotifyToast) => {
        ...
        this._dashboardService
          .deleteDashboard(
            this.dashboard.MA_DASHBOARD,
            this.dashboard.USER_MDF_ID
          )
          .pipe(takeUntil(this._unsubscribeAll))
          .subscribe((response: any) => {
